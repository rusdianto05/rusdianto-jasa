                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if ($order->user_buyer->detail_user->photo != NULL)
                                                    <img class="object-cover w-full h-full rounded-full" src="{{ url(Storage::url($order->user_buyer->detail_user->photo)) }}" alt="Foto buyer" loading="lazy" /> 
                                                @else
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-400">0{{ $order->user_buyer->detail_user->contact_number ?? '' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if ($order->service->thumbnail_service[0]->thumbnail != NULL)
                                                    <img class="object-cover w-full h-full rounded" src="{{ url(Storage::url($order->service->thumbnail_service[0]->thumbnail)) }}" alt="foto service" loading="lazy" />
                        ...
                        <div class="flex p-8 border border-gray-200 rounded-lg bg-serv-upload-bg h-128">
                            <div class="m-auto text-center">
                                ...
                                <div class="relative mt-0 md:mt-6">
                                    ...
                                    @else
                                    {{-- <label id="choose" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                        Choose File
                                    </label>  --}}
                                    <input type="file" accept=".zip" id="choose" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr" name="file" required>
                                    @endif
                            ...
                            <div class="">
                                <div class="p-1 mt-5">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            <textarea placeholder="Note for buyer..." type="text" name="note" id="note" autocomplete="note" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" rows="4" required {{ isset($order->file) ? 'disabled' : '' }}>{{ $order->note ?? '' }}</textarea>
                                            @if($errors->has('note'))
                                ...
                                <div class="px-1 py-4 text-right">
                                    ...
                                    onclick="return confirm('Are you sure want to back? any change you make will not be saved')">
                                        Back
                                     </a>
                                     @if ($order->file != NULL)
                                     <button class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" disabled>
                                       Data Approved
                                    </button> 
                                     @else
