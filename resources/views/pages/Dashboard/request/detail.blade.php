                                            ...
                                            <td class="px-1 py-5 text-sm w-2/8">
                                                <div class="flex items-center text-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-sm text-gray-400">0{{ $order->user_freelancer->detail_user->contact_number ?? '' }}</p>
                                                    </div>
                                                </div>
                                            </td>
                                                ...
                                                <div class="flex items-center text-sm">
                                                    <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                        @if ($order->service->thumbnail_service[0]->thumbnail != NULL)
                                                            <img class="object-cover w-full h-full rounded" src="{{ url(Storage::url($order->service->thumbnail_service[0]->thumbnail)) }}" alt="Photo service" loading="lazy" />
                                                        @else
                                                            <svg class="object-cover w-full h-full rounded text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                                                <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                                            </svg>  
                                        ...
                                        @if ($order->file != NULL)
                                            ...
                                            <div class="relative mt-0 md:mt-6">
                                                <a href="{{ url(Storage::url($order->file ?? '')) }}" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr"
                                                    onclick="return confirm('Are You Sure to Download this file?')">
                                                    Download File
                                                </a> 
                                            </div>
                                        @else
                                            <img src="{{ asset('assets/images/services-file-icon.svg') }}" alt="" class="w-20 mx-auto">
                                            <h2 class="mt-8 mb-1 text-2xl font-semibold text-gray-700">
                                                Maaf File Project Masih Kosong
                                            </h2>
                                            <p class="text-sm text-gray-400">
                                                Silahkan tunggu atau hubungi freelancer untuk upload hasil project
                                            </p>  
                                        @endif
                                ...
                                <form action="#" method="POST">
                                    <div class="">
                                        <div class="p-1 mt-5">
                                            <div class="grid grid-cols-6 gap-6">
                                                <div class="col-span-6">
                                                    ...
                                                    <textarea placeholder="Catatan dari freelancer..." type="text" name="note" id="note" autocomplete="note" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" rows="4" readonly>{{ $order->note ?? '' }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="px-1 py-4 text-right">
                                            ...
                                            onclick="return confirm('Are You Sure To Back? Any Change You Make Will Not Be Saved')">
                                                Back
                                            </a>
                                            @if ($order->order_status_id == 2 && isset($order->file))
                                                <a href="{{ route('member.approve.request', $order->id) }}" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                                                    onclick="return confirm('Are You Sure To Approve This Data?')">
                                                    Approve
                                                </a>
                                            @else
                                            <a type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                                                disabled>
                                                Approve
                                            </a>
                                            @endif
